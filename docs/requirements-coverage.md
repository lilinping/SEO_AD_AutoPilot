...
| E6-03 基础指标回传 | 已完成 | 已接通 Search Console/GA4 指标注入，并新增项目级指标快照历史查询接口 `/api/projects/{id}/metrics`；`MetricSnapshot` 现已补 `sourceMetricsSummary`，直接汇总 Search Console/GA4/AD 的主指标、认证来源与 fallback 原因；真实 provider 返回零 sessions/零 conversions 时保留真实零值，不再回填合成指标，并通过 `metricsComplete/metricsProvenance/metricWindow` 审计指标完整性与窗口 |
...
| 技术 SEO 修补 | 部分完成 | 已补发布前后 DOM/meta/schema 关键字段审计（含 patchAudit、verifiedPatch、patch manifest artifact 绑定）；新增技术 SEO/转化审计报告，显式标注实测与“待核验”发现、真实数据基线 7/28/90 天就绪度、归因数据缺口与 7/14/28 天复盘检查点；真实大规模外部写回覆盖仍需继续扩展 |
...
| 多语言、多站点、模板市场、A/B 实验 | 部分完成 | 已补 workspace 级 experiment policy / readiness report / settings editor，以及 localization policy / readiness report / settings editor、template market policy / readiness report / settings editor，用于控制实验分配、项目范围、locale clusters、模板目录和 rollout readiness；并新增 runtime assignment preview 接口与 project runtime-route 接口，以及 project runtime-route history / workspace runtime-route history / workspace runtime-route health report 与 settings/monitor/acceptance/project/dashboard 视图，可按 project/subject 解析 experiment variant，也可按 project/locale/host 解析 localization cluster，并把 experiment/localization/model gateway 汇总到 runtime route；runtime execute 已支持 redirect/render/proxy 与严格运行态门禁，边缘 proxy 会保留转发头和上游响应；现有 runtime-edge 导出、校验、rollout、probe、route override、worker batch 已进一步接入内置生产网关：控制面真实下发结构化 `siteRoutes`，网关以 API key 保护 `/deploy`、`/rollback`、`/deployments` 与 `/readyz`，按内容哈希生成幂等版本并原子持久化，项目级发布会合并其他站点，按 Host + 最长 `publicPath` 路由并剥离前缀后 reverse proxy；路由会校验 host、projectId 与控制面 proxy path 一致性，防止跨项目转发；生产可开启 DNS 解析、期望公网 IP 与 TLS 证书校验，新版本不就绪时记录 rejected 且保留上一活动快照，另支持人工恢复历史版本。软件层多站点 rewrite/reverse-proxy 发布与回滚编排已经闭环，但真实公网 DNS/TLS 切流仍需在目标生产环境执行；多语言、模板市场和 A/B 仍为策略/控制面能力，尚未全部形成业务生产闭环 |
| 真实广告平台接入与收益回传 | 部分完成 | 已接入 `ad_network` 真实探测与收益字段（providerFamily、providerName、providerRef、inventoryStatus、impressions、clicks、ctr、fillRate、rpm、estimated/settled daily、monthly、settlement window、currency、policy tier、payout threshold、geo coverage、provider program、revenue provenance、strict eligibility）并支持多 provider 字段别名归一化（如 AdSense/GAM/Mediavine/Ezoic/Freestar/Raptive/Monumetric/PubMatic/Seedtag/GumGum/Sovrn/Sharethrough/RevContent/Outbrain/Taboola/Yieldmo/Teads/Magnite/TripleLift/Index Exchange/Adform/Criteo/Undertone 类返回）；真实响应缺少展示、填充、RPM 或收益字段时不会再补造指标，而会标记 `metricsComplete=false`、列出缺失字段，并在 strictProviders 项目结算时以 `SETTLEMENT_AD_EVIDENCE_INCOMPLETE` 阻断；ad network 凭据现支持 `accessToken` 与 `credentialsJson/serviceAccountJson`，且支持 `authHeader` 自定义认证头；workspace settlement 执行在 strictProviders 模式下已绑定 ad_network 证据新鲜度，项目级结算若证据缺失或过期会被显式阻断并审计；同时新增 workspace/project 级 ad audit replay/history 视图，便于按项目回放 ad allowed / no-ad、revenue 与 failureCode；仍需对接更多真实广告平台与生产结算口径 |
...
- 独立 Playwright 视觉农场服务：受 API Key 与目标主机白名单保护，支持真实 baseline/preview PNG 截图、像素级差异、受保护产物下载；控制面可通过 `approvalRules.visualBaselineUrl/visualPreviewUrl` 提供明确的前后目标。
- 独立 Runtime Edge 网关：按 Host 从控制面路由表解析项目，仅回源同源 `/runtime-execute/proxy[-strict]` 路径，拒绝未知/重复 Host 与任意外部代理目标，并已加入 Compose 服务。

当前仍未完成的是：
- 真实外部数据源和真实写回链路。
- 生产级可观测性与告警。
- 视觉农场的容器级 Chromium smoke、生产部署和真实站点基线采集。
- 真实支付/结算网关、生产域名/DNS/TLS 接入及深度运行时 A/B 分流等后续版本能力。
